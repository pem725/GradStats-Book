...
d |>
  ...
  labs(title = "Blue points push covariance up; red points push it down") +
  theme_minimal()
```
...
ggplot(u, aes(x = x, y = y)) +
  ...
  labs(title = "r is near zero, but the relationship is perfect (just not linear)") +
  theme_minimal()
```
...
augment(m2) |>                     # broom: fitted values and residuals as columns
  ...
  geom_point(colour = "steelblue", size = 2) +
  labs(title = "Dataset 2 residuals arch — the misfit the scatter revealed",
       x = "fitted (predicted) value", y = "residual (y minus predicted)") +
  theme_minimal()
```
