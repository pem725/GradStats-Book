...
items = CSV.read("data/sim/ch28-curiosity.csv", DataFrame)

ev = reverse(eigvals(cor(Matrix(items))))    # eigenvalues, largest first

plot(1:length(ev), ev, marker = :circle, legend = false,
...
items = pd.read_csv("data/sim/ch28-curiosity.csv")
ev = np.sort(np.linalg.eigvals(items.corr()).real)[::-1]

plt.plot(range(1, len(ev) + 1), ev, marker="o", color="steelblue")
...
items = CSV.read("data/sim/ch28-curiosity.csv", DataFrame)
ev = reverse(eigvals(cor(Matrix(items))))
