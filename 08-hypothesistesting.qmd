...
tibble(
  standard_error = sigma / sqrt(n),           # standard error of the mean
  z              = (xbar - mu) / standard_error,  # how many SEs above the null?
  p_value        = pnorm(z, lower.tail = FALSE)   # P(mean > 22) if H0 is true
) |>
...
df = CSV.read("data/sim/ch07-nulldist.csv", DataFrame)

using Distributions
mu, sigma, n, xbar = 20, 4, 4, 22

standard_error = sigma / sqrt(n)                     # SE of the mean
z = (xbar - mu) / standard_error                     # how many SEs above the null
p_value = ccdf(Normal(0, 1), z)                      # P(mean > 22) if H0 is true
```
### Python
```{python}
import pandas as pd
df = pd.read_csv("data/sim/ch07-nulldist.csv")

from scipy.stats import norm

mu, sigma, n, xbar = 20, 4, 4, 22

standard_error = sigma / n**0.5                      # SE of the mean
z = (xbar - mu) / standard_error                     # how many SEs above the null
p_value = norm.sf(z)                                 # P(mean > 22) if H0 is true
```
:::

So a sample mean of 22 sits exactly one standard error above the null value, and the probability of getting a mean that large (or larger) *if the null is true* is about `r fmt_p(pnorm(1, lower.tail = FALSE))`. Let us see it:

::: {.panel-tabset}
### R
```{r}
#| label: fig-null-distribution
#| fig-cap: "Ten thousand sample means drawn from the null world, with the curve theory predicts: normal, centred on mu, with a standard deviation of sigma over the square root of n. The red line marks the mean we actually observed."
# Simulate 10,000 sample means from the H0 world and see where 22 lands
set.seed(1908)  # the year Gosset published "Student's" t

means <- tibble(
  sample_mean = map_dbl(1:10000, \(i) mean(rnorm(n, mean = mu, sd = sigma)))
)

ggplot(means, aes(x = sample_mean)) +
  geom_histogram(aes(y = after_stat(density)), bins = 40,
                 fill = "grey85", colour = "white") +
  # We did not fit this curve - theory says the sampling distribution of the
  # mean is normal, centred on mu, with SD equal to the standard error.
  stat_function(fun = dnorm, args = list(mean = mu, sd = sigma / sqrt(n)),
                colour = "firebrick", linewidth = 1) +
  geom_vline(xintercept = xbar, colour = "red", linewidth = 1.2) +
...
Random.seed!(1908)   # the year Gosset published "Student's" t

means = [mean(rand(Normal(20, 4), 4)) for _ in 1:10_000]

histogram(means, bins = 40, legend = false,
          title = "Sampling distribution of the mean IF the null is true",
          xlabel = "sample mean", ylabel = "count")
vline!([22], linewidth = 2, color = :red)
```
### Python
```{python}
import pandas as pd
...
rng = np.random.default_rng(1908)   # the year Gosset published "Student's" t
means = rng.normal(20, 4, size=(10_000, 4)).mean(axis=1)

plt.hist(means, bins=40, color="lightgrey", edgecolor="white")
plt.axvline(22, color="red", linewidth=2)
plt.title("Sampling distribution of the mean IF the null is true")
