...
```{r}
#| label: fig-z-standard
#| fig-cap: "The standard normal, with dotted lines marking z = -2, -1, 0, 1 and 2."
...
#| fig-cap: "Growth produced by twelve small multiplicative effects. Multiplying numbers close to 1 behaves almost exactly like adding them, so the bell appears here too."
set.seed(5)
...
```{r}
#| label: fig-z-tails
#| fig-cap: "The standard normal with the tails beyond z = plus or minus 1.96 shaded. Between them those tails hold 5% of the area."
...
A POMP of 0 is the lowest possible score, 100 the highest, and 50 the exact middle of the scale. Crucially, that meaning does not depend on the sample.

That fixed 0-to-100 range is what makes POMP comparable across measures, and it is also the one thing to remember about it when you come to *model* a POMP score. A z-score can wander anywhere; a POMP score cannot. It has a floor and a ceiling, real people sit exactly on them, and ordinary regression does not know that. We take that up properly in [Outcomes with a Floor and a Ceiling](34-bounded.qmd) - for now, just notice that the ruler we are recommending is a bounded one. Two measures with completely different response formats - a 1–5 scale and a 1–7 scale - both land on a common 0–100 scale you can read and compare at a glance:
