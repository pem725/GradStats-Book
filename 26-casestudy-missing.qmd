...
The code tabs below assume this chapter's data is already loaded. Grab the one-file setup for your language from [Getting the Book's Data](setup.qmd), run it once, then load what you need by name - this chapter uses `ch29-missing`. For example, `book_data("ch29-missing")` in R, Julia, or Python, or `!bookdata name = "ch29-missing".` in SPSS. Every language reads the same shipped files, so your numbers will match the ones printed here exactly.
:::
...
df = CSV.read("data/sim/ch29-missing.csv", DataFrame)
...
```{python}
import pandas as pd
df = pd.read_csv("data/sim/ch29-missing.csv")
