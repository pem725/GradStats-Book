...
The last case study takes on the part of every real dataset that the textbooks pretend away: the **holes**. People drop out of a two-year study. Respondents skip the touchy item. A sensor fails. In the purpose-versus-happiness project - a three-wave longitudinal study tracking hundreds of people over two years - attrition was not a footnote; it was a fork in the road, because *how you handle the missing data can change the answer.*
...
Recall the three mechanisms from the [data-cleaning chapter](05-datacleaning.qmd) - they are the whole ballgame:
...
The complete-case mean is badly **biased downward**. Of course it is: we systematically dropped the high-`z` people, and high `z` means high `y`, so the survivors are not representative. The hole did not just cost us sample size - it *moved the answer*.
...
Here is the subtlety that separates a careful analyst from a careless one. The *marginal mean* of `y` was wrecked - but watch the *regression slope* of `y` on `z`:
...
Our single imputation recovered the mean, but it quietly cheated on one thing: it treated the filled-in values as if they were real, understating our uncertainty. The fix is **multiple imputation** - impute the holes several times (each with fresh noise), analyze each completed dataset, and **pool** the results with Rubin's rules, so the between-imputation variability is folded into the standard errors. In R that is the `mice` package:
...
The purpose-versus-happiness study did exactly this - fitting its growth models under **complete-case and under multiple imputation side by side** - and reported both, so a reader could see whether the two-year conclusions survived the missing data. That comparison *is* the analysis; presenting only the complete-case result and hoping would have been a choice to look away.
...
That closes our apprenticeship in the wild - four real studies, every judgment call on the table: a total score caught hiding information, a measure built and confirmed and preregistered, and a dataset's holes bent (and not bent) an estimate. The tools were never the hard part. Knowing which one, when, and why - and admitting what you cannot know - is the whole job.
