...
d = CSV.read("data/sim/ch01-heights.csv", DataFrame)   # the book's heights
...
d = pd.read_csv("data/sim/ch01-heights.csv")   # the book's heights
...
```{julia}
using CSV, DataFrames, Plots, Distributions
d = CSV.read("data/sim/ch01-skewed.csv", DataFrame)   # the book's skew-normal
alpha = 6                                             # the lean it was built with

histogram(d.value, bins = 40, normalize = :pdf, legend = false,
          xlabel = "value", ylabel = "density")
# Distributions.jl ships the skew-normal, so the curve is one call
plot!(x -> pdf(SkewNormal(0, 1, alpha), x), -2, 4,
...
d = pd.read_csv("data/sim/ch01-skewed.csv")   # the book's skew-normal
value = d.value
alpha = 6                                    # the lean it was built with
...
d = CSV.read("data/sim/ch01-shapes.csv", DataFrame)   # the book's draws

shapes = [n => d[!, n] for n in ["Normal", "Uniform", "Binomial", "Poisson"]]
...
d = pd.read_csv("data/sim/ch01-shapes.csv")   # the book's draws
shapes = {n: d[n] for n in ["Normal", "Uniform", "Binomial", "Poisson"]}
