...
```r
install.packages(c("tidyverse", "broom", "plotly", "betareg", "knitr", "rmarkdown"))
```
...
`betareg` is the one modelling package the book actually runs, in the [bounded-outcomes chapter](34-bounded.qmd), and even there it arrives second: the chapter fits the beta regression from scratch first and uses `betareg` to check that the fifteen hand-written lines got the same answer.

Otherwise the **statistical engines are still base R** - `lm()`, `aov()`, `glm()`, `factanal()`, `prcomp()`, `power.t.test()` - and so is every from-scratch derivation: reliability computed by hand, the Rasch estimator, the EM algorithm for latent classes, the bootstrap and permutation tests. That is deliberate. The tidyverse is how we *handle and display* the data; the statistics are demonstrated with tools that ship with R, so nothing important is hidden inside a package.
...
```julia
using Pkg
Pkg.add(["CSV", "DataFrames", "CategoricalArrays", "Distributions", "GLM",
         "HypothesisTests", "MixedModels", "MultivariateStats", "Optim",
         "Plots", "PlotlyJS", "Roots", "StatsBase", "StatsModels"])
```
