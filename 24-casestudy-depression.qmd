...
* Concatenate the items into a single key, then count distinct keys.
STRING profile (A63).
COMPUTE profile = CONCAT(STRING(s1,F1), '-', STRING(s2,F1), '-', STRING(s3,F1)).
* ... continue for s4 to s21 ...
EXECUTE.
...
nrow(bdi), length(unique(key)),
100 * mean(values(countmap(key))[indexin(key, collect(keys(countmap(key))))] .== 1)
```
### Python
```python
...
### SPSS
```
* Ordering violations: count pairs where a person scores a "harder"
* (rarely endorsed) symptom above an "easier" one.
* SPSS has no vectorised pairwise comparison, so this needs nested loops
* over the 21 x 21 item pairs inside a VECTOR.
DESCRIPTIVES VARIABLES=s1 TO s21 /STATISTICS=MEAN.

VECTOR s = s1 TO s21.
VECTOR d = d1 TO d21.        /* item means, filled in from the run above */
COMPUTE ovi = 0.
LOOP #i = 1 TO 21.
  LOOP #j = 1 TO 21.
    IF (d(#i) < d(#j) AND s(#i) > s(#j)) ovi = ovi + 1.
  END LOOP.
END LOOP.
EXECUTE.

FREQUENCIES VARIABLES=ovi /STATISTICS=MEDIAN MAXIMUM.
CORRELATIONS /VARIABLES=ovi total.
```
### Julia
```julia
using Statistics
difficulty = [mean(col) for col in eachcol(bdi)]   # higher = more endorsed

ovi = [sum(difficulty[i] < difficulty[j] && r[i] > r[j]
           for i in 1:k, j in 1:k) for r in eachrow(bdi)]

median(ovi), maximum(ovi), cor(ovi, totals)        # NOT just severity
```
### Python
```python
difficulty = bdi.mean().values          # higher = more commonly endorsed

harder = difficulty[:, None] < difficulty[None, :]
X = bdi.values
ovi = np.array([((X[r][:, None] > X[r][None, :]) & harder).sum()
                for r in range(len(X))])

np.median(ovi), ovi.max(), np.corrcoef(ovi, totals)[0, 1]   # NOT just severity
```
:::
