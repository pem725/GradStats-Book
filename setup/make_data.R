...
# Every dataset is written twice: once as a CSV that all four languages read,
# and once as a tiny .sps file holding the GET DATA command that loads it.
# SPSS's GET DATA /TYPE=TXT will not infer columns from a header row - it
...
put <- function(x, name) {
  ...
  writeLines(c(
    paste0("* Loads ", name, ".csv. Generated by setup/make_data.R - do not hand-edit."),
    # a RELATIVE path, so the files work on whoever's machine downloads them;
    # run SPSS with the book's folder as the working directory
    paste0("GET DATA /TYPE=TXT /FILE='data/sim/", name, ".csv'"),
    "  /DELIMITERS=',' /QUALIFIER='\"' /FIRSTCASE=2",
    paste0("  /VARIABLES=", paste(spss_name, spss_fmt, collapse = " "), ".")
  ), file.path("data/sim", paste0(name, ".sps")))
