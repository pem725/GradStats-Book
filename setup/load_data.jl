# ---------------------------------------------------------------------------
# load_data.jl - the book's datasets, in Julia.
#
#     include("setup/load_data.jl")
...
# broke. The recipe for each dataset is listed in RECIPE, so you can still see
# exactly how it was built.
# ---------------------------------------------------------------------------

using CSV, DataFrames, CategoricalArrays

const DATA_DIR = "data/sim"

# Categorical order matters and CSV does not preserve it. Without these, Julia
...
# alphabetical ladder.
const LEVELS = Dict(
  "ch03-spread"     => Dict(:group => ["tight (sd=3)", "wide (sd=20)"]),
  "ch12-groups"     => Dict(:group => ["control", "treatment"]),
  "ch16-pets"       => Dict(:pet  => ["cat", "fish", "pig", "dog"],
                            :mess => ["messy", "tidy"]),
  "ch17-dose"       => Dict(:dose => ["none", "low", "high"]),
  "ch18-pets"       => Dict(:pet  => ["cat", "fish", "pig", "dog"]),
  "ch18-unbalanced" => Dict(:petw => ["cat", "fish", "pig", "dog"]),
  "ch21-invariance" => Dict(:grp  => ["A", "B"]),
  "appendix-d"      => Dict(:group => ["A", "B"]),
)
...
const RECIPE = Dict(
  ...
  "ch02-income"           => "c(30, 35, 40, 42, 45, 48, 5000) - one very rich person",
  "ch03-spread"           => "set.seed(7); rnorm(500,100,sd=3) and rnorm(500,100,sd=20)",
  "ch03-x"                => "c(3, 5, 5, 7, 8, 9, 12, 40)",
  "ch04-dirty"            => "Seven rows: impossible age, negative income, a 999 code, a stray 'b'.",
  ...
  "ch09-ctt"              => "set.seed(8); truth ~ N(100,15), error ~ N(0,8)",
  ...
  "ch09-attenuation"      => "set.seed(10); true pair correlated .6, seen through noisy sensors",
  "ch10-validity"         => "set.seed(4); a new scale, an established scale, an unrelated measure",
  ...
  "ch15-tangled"          => "set.seed(1936); y, x1, x2 all built from a shared 'base'",
  ...
  "ch21-invariance"       => "set.seed(1); same true trait, group B answers 0.5 higher",
  "ch22-lca"              => "set.seed(2026); two latent classes differing in PATTERN",
  ...
  "ch27-bdi"              => "set.seed(2026); 1200 people, 21 graded 0-3 items",
...
function book_data(name::AbstractString)
    d = CSV.read(joinpath(DATA_DIR, name * ".csv"), DataFrame)
    for (col, levels) in get(LEVELS, name, Dict())
        d[!, col] = categorical(d[!, col]; levels = levels)
    end
    return d
end

"Print how a dataset was built."
recipe(name) = println(name, ": ", get(RECIPE, name, "no recipe recorded"))

"List every dataset the book ships."
datasets() = sort(collect(keys(RECIPE)))
