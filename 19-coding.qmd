...
### Julia
```{julia}
using CSV, DataFrames, CategoricalArrays, GLM, StatsModels, Statistics
df = CSV.read("data/sim/ch18-pets.csv", DataFrame)
df.pet = categorical(df.pet; levels = ["cat", "fish", "pig", "dog"])
...
df = pd.read_csv("data/sim/ch18-pets.csv")
df["pet"] = pd.Categorical(df.pet, categories=["cat","fish","pig","dog"])

df.groupby("pet", observed=True)["happy"].mean()
```
:::
...
m_eff = smf.ols("happy ~ C(pet, Sum)", data=df).fit()   # each group vs grand mean
...
m_con = lm(@formula(happy ~ pet), df,
           contrasts = Dict(:pet => HypothesisCoding(
               ...
               levels = ["cat", "fish", "pig", "dog"],
               labels = ["usual_vs_unusual", "cat_vs_fish", "pig_vs_dog"])))
coeftable(m_con)
```
### Python
```{python}
...
m_nonsense = lm(@formula(happy ~ pet), df,
                contrasts = Dict(:pet => HypothesisCoding(
                    [3.0 -7.0 2.5 0.1; 1.0 1.0 -2.0 4.0; -5.0 2.0 8.0 -1.0];
                    levels = ["cat", "fish", "pig", "dog"],
                    labels = ["huh", "wat", "lol"])))
coeftable(m_nonsense)
```
### Python
```{python}
import pandas as pd
...
```{julia}
using CSV, DataFrames, CategoricalArrays, GLM, StatsModels, Statistics
d3 = CSV.read("data/mod3data.csv", DataFrame)
# f3 is coded 2,3,4,5 - R's factor(labels=) maps the SORTED values in order
d3.grp = categorical(map(f -> Dict(2=>"A", 3=>"B", 4=>"C", 5=>"D")[f], d3.f3);
                     levels = ["A", "B", "C", "D"])

combine(groupby(d3, :grp), :y1 => mean, nrow => :n)
```
### Python
```{python}
import pandas as pd
...
# f3 is coded 2,3,4,5 - R's factor(labels=) maps the SORTED values in order
d3["grp"] = pd.Categorical(d3.f3.map({2:"A", 3:"B", 4:"C", 5:"D"}),
                           categories=["A","B","C","D"])

d3.groupby("grp", observed=True)["y1"].agg(["mean", "size"])
```
:::
...
m_real_eff = lm(@formula(y1 ~ grp), d3,
                contrasts = Dict(:grp => EffectsCoding()))
coeftable(m_real_eff)
```
### Python
```{python}
import pandas as pd
...
m_real_eff.params.iloc[0]      # intercept = unweighted mean of the four means
```
:::
