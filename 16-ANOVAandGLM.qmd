...
smf.ols("happy ~ pet", data=df).fit().params.round(3)
```
:::
...
tibble(intercept = eff[1], grand_mean = GM) |> round2()
```
...
tibble(coef1 = eff[2], cat_minus_GM = means$mean[1] - GM) |> round2()
```
...
```python
import statsmodels.formula.api as smf
smf.ols("sad ~ pet * mess", data=df).fit().params.round(3)
```
:::
...
```python
import statsmodels.formula.api as smf
smf.logit("passed ~ x", data=df).fit().params.round(3)
```
:::
