...
# Create the 3D helix. Transparent backgrounds + a light font let it sit
# cleanly on the book's dark theme.
plot_ly(helix_df,
        x = ~x, y = ~y, z = ~z,
        type = "scatter3d",
        mode = "lines",
...
Error is the gap between what we observed and what we predicted: $e = \text{observed} - \text{predicted}$. Do not read "error" as "mistake." It is the part of the world our prediction failed to capture, and taming it - shrinking it, understanding it, quantifying it - is most of what statistics actually does.
:::

::: callout-important
## Probability (likelihood of a particular event occurring)

Probability puts a number on how likely an event is, scaled from 0 (never happens) to 1 (certain). It is the currency we use to buy and sell uncertainty. Every claim we make later - every p-value, every confidence interval - is denominated in probability.
:::
