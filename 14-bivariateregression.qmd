...
```{r}
#| label: fig-toy-points
#| message: false
#| fig-cap: "Five points, few enough that every number in this chapter can be checked by hand."
...
#| label: fig-toy-fit
#| message: false
#| fig-cap: "The same five points with the least-squares line through them."
lm1 <- lm(y ~ x, data = toy)
...
df = CSV.read("data/sim/ch14-toy.csv", DataFrame)
...
```{python}
import pandas as pd
df = pd.read_csv("data/sim/ch14-toy.csv")
...
#| label: fig-residuals
#| message: false
#| fig-cap: "The residuals drawn as vertical segments. Least squares is the line that makes the sum of their squares as small as it can be."
...
#| label: fig-gpa
#| message: false
#| fig-cap: "College GPA against high-school GPA for 100 students, with the fitted line."
...
lm3 = lm(@formula(CollegeGPA ~ HSGPA), gpa)
coeftable(lm3)
```
### Python
```{python}
