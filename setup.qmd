...
The zip unpacks to one `GradStats-SPSS` folder containing the data and one `.sps` file per chapter. Double-click **`START-WINDOWS.cmd`** on Windows or **`START-MAC.command`** on a Mac. The starter finds that folder's location, prepares the chapter files, and opens a short setup check. Run the check in SPSS; its mean height should be about 67.99. Then open any chapter file in `spss/` and run it on its own. There is no path to type or edit. If you move the folder later, run the starter again. You do not need the separate data zip.
...
No SPSS licence? **[PSPP](https://www.gnu.org/software/pspp/)** is a free, open-source program that runs much SPSS syntax, from a GUI or the command line. Some IBM SPSS commands in the book are beyond PSPP's command set, so a successful book build alone does not verify those blocks in IBM SPSS.
:::
