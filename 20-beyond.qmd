...
::: {.callout-important}
## Why the four tabs in this chapter do *not* agree

Everywhere else in this book, the R, SPSS, Julia and Python tabs print the
same numbers, because they read the same shipped data. This chapter is the
exception, and the reason is the point of the chapter.

Resampling gets its answer by drawing at random - thousands of times. The
*sample* is shared here, exactly as elsewhere. The *draws* are not, because
each language has its own random number generator, and no seed makes them
agree. So the bootstrap standard error will differ in the third digit
between tabs, and the permutation p-value will wobble around.

That is not a defect to apologise for. It is the honest face of the method:
**run a bootstrap twice and you get two slightly different answers.** If
that bothers you, raise the number of resamples and watch the disagreement
shrink - which is itself the best demonstration of what those 2000 replicates
are buying you. What must not move is the conclusion.
:::
...
### Julia
```{julia}
using CSV, DataFrames, Random, Statistics, StatsBase
Random.seed!(42)
samp = CSV.read("data/sim/ch25-skewed-sample.csv", DataFrame).samp
...
samp = pd.read_csv("data/sim/ch25-skewed-sample.csv").samp.to_numpy()
...
### Julia
```{julia}
using CSV, DataFrames, Random, Statistics, StatsBase, Plots
Random.seed!(42)
samp = CSV.read("data/sim/ch25-skewed-sample.csv", DataFrame).samp
boot_means = [mean(sample(samp, length(samp), replace = true)) for _ in 1:2000]
...
Random.seed!(7)
d = CSV.read("data/sim/ch25-twogroups.csv", DataFrame)
value, group = d.value, d.group
...
d = pd.read_csv("data/sim/ch25-twogroups.csv")
value, group = d.value.to_numpy(), d.group.to_numpy()
...
jk.mean(), np.sqrt((n - 1) * ((jk - jk.mean())**2).mean())   # mean and SE
```
:::
...
z = CSV.read("data/sim/ch25-outlier.csv", DataFrame).z
...
z = pd.read_csv("data/sim/ch25-outlier.csv").z.to_numpy()
...
theta = range(0, 1, length = 1000)
prior = pdf.(Beta(2, 2), theta)
like  = [pdf(Binomial(9, t), 6) for t in theta]
post  = like .* prior; post ./= sum(post)
...
using Distributions, StatsBase, Statistics
theta = range(0, 1, length = 1000)
post  = [pdf(Binomial(9, t), 6) for t in theta] .* pdf.(Beta(2, 2), theta)
post ./= sum(post)
draws = sample(collect(theta), Weights(post), 100_000)
...
theta = np.linspace(0, 1, 1000)
post = binom.pmf(6, 9, theta) * beta.pdf(theta, 2, 2); post /= post.sum()
rng = np.random.default_rng()
draws = rng.choice(theta, 100_000, p=post)
