# Reliability (not Validity) of Data

Let's introduce an idea that reframes the whole enterprise: your **measures are sensors**. A questionnaire, a stopwatch, a bathroom scale - each is a device that reads out a number *about* something, and like any sensor it is imperfect. The number is not the truth; it is the truth plus some noise. Before we trust a measurement to carry an argument, we must ask two separate questions: is it **consistent** (reliability, this chapter), and is it **on target** (validity, the next)? These are not the same question, and confusing them wrecks research.

## Learning Objectives

1.  Understand the classical test theory model: observed = true + error.
2.  Define reliability as a ratio of variances.
3.  Know the main ways to estimate reliability and when each applies.
4.  See how unreliability *attenuates* the relationships you care about.

## Observed = True + Error

Classical test theory rests on one deceptively simple equation:

$$O = T + E$$

Every **observed** score ($O$) is a person's **true** score ($T$) - what we actually want - plus **error** ($E$), the sensor's noise. Because true score and random error are independent, the same decomposition holds for their *variances*:

$$\text{var}(O) = \text{var}(T) + \text{var}(E)$$

Observed-score variance is always larger than true-score variance, because error only ever *adds* spread. Our whole goal in measurement is to make the error term small.

## Reliability Is a Ratio of Variances

**Reliability** ($\rho$) is the fraction of the observed variance that is *real* - the signal share:

$$\rho = \frac{\text{var}(T)}{\text{var}(O)} = \frac{\text{var}(T)}{\text{var}(T) + \text{var}(E)}$$

It runs from 0 (all noise) to 1 (all signal).

::: {.callout-note}
## From the Field
Reliability is necessary but not sufficient: a consistent measure can still be *meaningless* until it is anchored to something real. Sechrest, McKnight, and McKnight [@SechrestMcKnight1996] made exactly this argument for psychotherapy-outcome measures - **calibration**, not just consistency, is what lets a score be interpreted. It is the same warning the dartboard makes later in this chapter, made in print.
:::

Let's build a world where we *know* the true scores and confirm the definition:

::: {.callout-note}
## Working in SPSS, Julia, or Python?
The code tabs below assume this chapter's data is already loaded. Grab the one-file setup for your language from [Getting the Book's Data](setup.qmd), run it once, then load what you need by name - this chapter uses `ch09-ctt`, `ch09-items-tau`, `ch09-items-congeneric`, `ch09-attenuation`. For example, `book_data("ch09-ctt")` in R, Julia, or Python, or `!bookdata name = "ch09-ctt".` in SPSS. Every language reads the same shipped files, so your numbers will match the ones printed here exactly.

The real 10-item scale is `data/moddat2.csv`.
:::

::: {.panel-tabset}
### R
```{r}
#| message: false
library(tidyverse)    # dplyr, ggplot2, purrr, tibble, readr, stringr, forcats
source("_common.R")   # book-wide helpers: round2(), fmt_p(), tidy2()

set.seed(8)
ctt <- tibble(
  truth = rnorm(1000, 100, 15),    # the true scores we wish we could see
  error = rnorm(1000, 0, 8)        # sensor noise
) |>
  mutate(observed = truth + error) # what we actually record

ctt |>
  summarise(reliability = var(truth) / var(observed)) |>
  round2()
```
### SPSS
```{pspp}
* The book's own true/error/observed scores. Reliability is the true-score
* share of the observed variance, read off the two variances below.
INSERT FILE='data/sim/ch09-ctt.sps'.

DESCRIPTIVES VARIABLES=truth observed /STATISTICS=VARIANCE.
```
### Julia
```{julia}
using CSV, DataFrames, Statistics
ctt = CSV.read("data/sim/ch09-ctt.csv", DataFrame)

round(var(ctt.truth) / var(ctt.observed), digits = 2)   # true-score share
```
### Python
```{python}
import pandas as pd
ctt = pd.read_csv("data/sim/ch09-ctt.csv")

round(float(ctt.truth.var() / ctt.observed.var()), 2)  # true-score share
```
:::

About 0.80 of the variance in our observed scores is real signal; the rest is sensor noise. We built these data with a true ratio of $225 / 289 = 0.78$, and this sample came out a shade above it - a reminder that reliability is an estimate like any other, and estimates wobble. In practice we can never see `truth`, so we *estimate* reliability from clever comparisons.

## Four Ways to Estimate Reliability

1.  **Test–retest** - measure the same people twice and correlate. High correlation means the sensor gives a stable reading over time. (Careful: real change looks like unreliability.)
2.  **Internal consistency** - do the items of a scale agree with each other? Summarized by **Cronbach's alpha**.
3.  **Split-half** - split the items in two, score each half, correlate. A quick internal-consistency check.
4.  **Inter-rater** - do two human coders agree? Essential whenever judgment is the sensor.

### Cronbach's Alpha, By Hand

Alpha asks: relative to the total score's variance, how small are the individual item variances? If items measure the same thing, the whole is much more than the sum of its jittery parts.

$$\alpha = \frac{k}{k-1}\left(1 - \frac{\sum \text{var(item)}}{\text{var(total)}}\right)$$

::: {.panel-tabset}
### R
```{r}
set.seed(9)
# simulate 6 items that share a common "true" factor plus item-specific noise
common <- rnorm(500)
items <- map(1:6, \(i) common + rnorm(500)) |>   # 500 people x 6 items
  set_names(paste0("item", 1:6)) |>
  as_tibble()

k         <- ncol(items)
var_items <- items |> summarise(across(everything(), var)) |> sum()
var_total <- var(rowSums(items))

tibble(cronbach_alpha = (k / (k - 1)) * (1 - var_items / var_total)) |> round2()
```
### SPSS
```{pspp}
INSERT FILE='data/sim/ch09-items-tau.sps'.
* Cronbach's alpha for six items sharing a common factor.
* NOTE (for Jeff): this is the section reserved for your SPSS review -
* RELIABILITY is the workhorse, and its output reports raw alpha plus the
* alpha-if-item-deleted table the R tab builds by hand further down.
RELIABILITY
  /VARIABLES=item1 item2 item3 item4 item5 item6
  /MODEL=ALPHA
  /SUMMARY=TOTAL.
```
### Julia
```{julia}
using CSV, DataFrames, Statistics
items = CSV.read("data/sim/ch09-items-tau.csv", DataFrame)

k = ncol(items)
var_items = sum(var(col) for col in eachcol(items))
var_total = var(sum(eachcol(items)))

round((k / (k - 1)) * (1 - var_items / var_total), digits = 2)
```
### Python
```{python}
import pandas as pd
items = pd.read_csv("data/sim/ch09-items-tau.csv")

k = items.shape[1]
var_items = items.var().sum()
var_total = items.sum(axis=1).var()

round(float((k / (k - 1)) * (1 - var_items / var_total)), 2)
```
:::

An alpha around 0.85 says the six items hang together well; they are largely reading the same underlying construct. By rough convention, 0.70 is "acceptable," 0.80+ "good," but do not lean on the threshold too hard: alpha climbs with more items whether or not they are any good, so a high alpha is necessary, not sufficient.

## Measurement Models: How Equal Are Your Items?

Cronbach's alpha, which we just computed, quietly assumes something about the items, and whether that assumption holds decides whether alpha is right or too low. There is a small hierarchy of **measurement models**, each making a stronger claim about how the items relate to the true score.

- **Parallel** - every item measures the true score with the *same* loading *and* the *same* error variance. All items are interchangeable, equally good rulers, with equal observed variances. This is the strictest model, and it is what test-retest and alternate-forms reliability assume.
- **Tau-equivalent** - every item has the same loading (a one-unit change in the true score moves each item equally), but the error variances may differ. This is the assumption **Cronbach's alpha depends on**: alpha is exact only when the items are (essentially) tau-equivalent.
- **Congeneric** - items may have *different* loadings *and* different error variances; they share one latent factor but on their own scales. This is the most realistic model, and it describes most real scales, where some items are simply better indicators than others.

Why it matters: when items are congeneric (unequal loadings), **alpha underestimates reliability**, and **McDonald's omega** ($\omega$) is the right coefficient. Omega is computed from a factor model, so it can weight items by how well they actually measure the construct. Watch alpha fall short of omega when the loadings differ:

::: {.panel-tabset}
### R
```{r}
set.seed(3); N <- 500
Tval   <- rnorm(N)
lambda <- c(0.9, 0.8, 0.7, 0.5, 0.4)      # congeneric: unequal item quality

items <- map(lambda, \(l) l * Tval + rnorm(N, sd = sqrt(1 - l^2))) |>
  set_names(paste0("item", seq_along(lambda))) |>
  as_tibble()

k     <- ncol(items)
alpha <- (k / (k - 1)) *                  # assumes tau-equivalence
  (1 - sum(summarise(items, across(everything(), var))) / var(rowSums(items)))

fa    <- factanal(items, 1)               # a one-factor model gives the loadings
lam   <- as.numeric(fa$loadings)
psi   <- fa$uniquenesses
omega <- sum(lam)^2 / (sum(lam)^2 + sum(psi))    # McDonald's omega

tibble(cronbach_alpha = alpha, mcdonald_omega = omega) |> round2()
```
### SPSS
```{pspp}
INSERT FILE='data/sim/ch09-items-congeneric.sps'.
* Alpha assumes tau-equivalence; with unequal loadings it UNDERSTATES
* reliability, and McDonald's omega is the right coefficient.
* RELIABILITY gives alpha directly.
RELIABILITY
  /VARIABLES=item1 item2 item3 item4 item5
  /MODEL=ALPHA.

* Omega needs the factor loadings. Base SPSS has no omega keyword, so you
* extract a one-factor solution and compute omega from its output:
*   omega = (SUM lambda)^2 / ((SUM lambda)^2 + SUM psi)
* In SPSS the extraction line reads /EXTRACTION=ML, matching the R tab.
* PSPP offers PAF, PC and PA1 but not ML, so principal axis factoring runs
* here; on this data the loadings agree to the two decimals we print.
FACTOR
  /VARIABLES=item1 item2 item3 item4 item5
  /EXTRACTION=PAF
  /CRITERIA=FACTORS(1)
  /PRINT=INITIAL EXTRACTION.
```
### Julia
```{julia}
using CSV, DataFrames, Statistics, MultivariateStats
items = CSV.read("data/sim/ch09-items-congeneric.csv", DataFrame)

k = ncol(items)
alpha = (k / (k - 1)) *
        (1 - sum(var(c) for c in eachcol(items)) / var(sum(eachcol(items))))

# standardise first: R's factanal factors the CORRELATION matrix, so fitting
# the raw covariances here would disagree with the R tab in the second decimal
X = Matrix(items)
Z = (X .- mean(X, dims = 1)) ./ std(X, dims = 1)

fa  = fit(FactorAnalysis, Z'; maxoutdim = 1)
lam = vec(loadings(fa))
psi = var(fa)          # keep on its own line: a "lam = ...; psi = ..." line
                       # takes the value of its last statement and echoes it
omega = sum(lam)^2 / (sum(lam)^2 + sum(psi))

round.((alpha, omega), digits = 2)
```
### Python
```{python}
import numpy as np, pandas as pd
from sklearn.decomposition import FactorAnalysis
items = pd.read_csv("data/sim/ch09-items-congeneric.csv")

k = items.shape[1]
alpha = (k / (k - 1)) * (1 - items.var().sum() / items.sum(axis=1).var())

# standardise first: R's factanal factors the CORRELATION matrix, so fitting
# the raw covariances here would disagree with the R tab in the second decimal
z = (items - items.mean()) / items.std()

fa = FactorAnalysis(n_components=1).fit(z)
lam = fa.components_.ravel(); psi = fa.noise_variance_
omega = lam.sum()**2 / (lam.sum()**2 + psi.sum())

round(float(alpha), 2), round(float(omega), 2)
```
:::

Omega is the larger, and more honest, number. When the items are truly tau-equivalent (equal loadings), the two coincide; the more the loadings differ, the more alpha understates the reliability. The rule the authors follow: **report omega, because it does not assume tau-equivalence.** Alpha is a fine quick check, but it is a lower bound, not the last word.

## Why You Should Care: Attenuation

Now the practical consequence. Unreliable measures **attenuate** (shrink) the correlations you are trying to detect. A real relationship between two constructs looks weaker than it is, purely because your sensors are noisy:

::: {.panel-tabset}
### R
```{r}
set.seed(10)
att <- tibble(Tx = rnorm(1000)) |>
  mutate(
    Ty = 0.6 * Tx + rnorm(1000, 0, 0.8),   # true correlation ~ .6
    Ox = Tx + rnorm(1000, 0, 1),           # now observe each through a
    Oy = Ty + rnorm(1000, 0, 1)            # noisy sensor
  )

att |>
  summarise(true_r = cor(Tx, Ty), observed_r = cor(Ox, Oy)) |>
  round2()
```
### SPSS
```{pspp}
* Attenuation: unreliable sensors shrink the correlation you can observe.
INPUT PROGRAM.
LOOP #i = 1 TO 1000.
  COMPUTE Tx = RV.NORMAL(0, 1).
  COMPUTE Ty = 0.6 * Tx + RV.NORMAL(0, 0.8).
  COMPUTE Ox = Tx + RV.NORMAL(0, 1).
  COMPUTE Oy = Ty + RV.NORMAL(0, 1).
  END CASE.
END LOOP.
END FILE.
END INPUT PROGRAM.
EXECUTE.

* The true correlation, then the one a noisy sensor actually reports.
CORRELATIONS /VARIABLES=Tx Ty.
CORRELATIONS /VARIABLES=Ox Oy.
```
### Julia
```{julia}
using CSV, DataFrames, Statistics
att = CSV.read("data/sim/ch09-attenuation.csv", DataFrame)

round.((cor(att.Tx, att.Ty), cor(att.Ox, att.Oy)), digits = 2)   # true, then observed
```
### Python
```{python}
import pandas as pd
att = pd.read_csv("data/sim/ch09-attenuation.csv")

round(float(att.Tx.corr(att.Ty)), 2), round(float(att.Ox.corr(att.Oy)), 2)  # true, observed
```
:::

The observed correlation is noticeably smaller than the true one. A real effect can hide behind a bad ruler, which means a null result might be a measurement failure rather than the absence of an effect. Reliability is more than bookkeeping; it can be the difference between finding your effect and missing it.

## A Real Scale, End to End

Enough simulation. Let's run the whole pipeline on real data: a 10-item scale (`x1`–`x10`) from the graduate GLM course (`data/moddat2.csv`, $n = 30$). Three moves: **key** the items, **estimate** $\alpha$, and check **which items earn their place**.

Some items are worded in reverse, so a high true standing produces a *low* score - flip them first or they fight the rest of the scale and sink $\alpha$. We spot a reverse item by its *negative* item-rest correlation (it correlates the wrong way with the sum of the others):

::: {.panel-tabset}
### R
```{r}
d <- read_csv("data/moddat2.csv", show_col_types = FALSE)
X <- d |> select(num_range("x", 1:10))

alpha_keyed <- function(items) {
  total <- rowSums(items)
  # item-rest correlation: each item against the sum of the *other* items
  r     <- map_dbl(items, \(col) cor(col, total - col))
  keys  <- if_else(r < 0, -1, 1)              # flip the reverse-worded items

  scored <- map2(items, keys, \(col, key) col * key) |> as_tibble()
  k      <- ncol(scored)
  alpha  <- (k / (k - 1)) *
    (1 - sum(summarise(scored, across(everything(), var))) / var(rowSums(scored)))

  list(alpha = alpha, keys = keys, scored = scored)
}

a <- alpha_keyed(X)
a$keys                                        # -1 marks a reverse-scored item
tibble(cronbach_alpha = a$alpha) |> round2()  # alpha for the keyed scale
```
### SPSS
```
* A real 10-item scale: reverse-key the items that correlate negatively
* with the rest, then report alpha. RELIABILITY does the keying check for
* you through the corrected item-total correlations - any NEGATIVE value
* flags an item to reverse before you trust the alpha.
GET DATA /TYPE=TXT /FILE='data/moddat2.csv'
  /DELIMITERS=',' /QUALIFIER='"' /FIRSTCASE=2
  /VARIABLES=x1 F16.6 x2 F16.6 x3 F16.6 x4 F16.6 x5 F16.6 x6 F16.6 x7 F16.6
             x8 F16.6 x9 F16.6 x10 F16.6 sex A8 y1 F16.6 y2 F16.6.

RELIABILITY
  /VARIABLES=x1 x2 x3 x4 x5 x6 x7 x8 x9 x10
  /MODEL=ALPHA
  /SUMMARY=TOTAL.

* Reverse any flagged item (here shown for a 1-5 response scale), then rerun:
* RECODE x3 (1=5)(2=4)(3=3)(4=2)(5=1) INTO x3r.
```
### Julia
```{julia}
using CSV, DataFrames, Statistics
d = CSV.read("data/moddat2.csv", DataFrame)
X = select(d, r"^x\d+$")

function alpha_keyed(items)
    total = sum(eachcol(items))
    r = [cor(col, total .- col) for col in eachcol(items)]   # item-rest
    keys = [x < 0 ? -1 : 1 for x in r]                       # flip reverse items
    scored = DataFrame([n => c .* k for ((n, c), k) in zip(pairs(eachcol(items)), keys)])
    k_ = ncol(scored)
    a = (k_ / (k_ - 1)) *
        (1 - sum(var(c) for c in eachcol(scored)) / var(sum(eachcol(scored))))
    (alpha = a, keys = keys, scored = scored)
end

a = alpha_keyed(X)
a.keys                            # -1 marks a reverse-scored item
round(a.alpha, digits = 2)        # alpha for the keyed scale
```
### Python
```{python}
import pandas as pd, numpy as np

d = pd.read_csv("data/moddat2.csv")
X = d[[f"x{i}" for i in range(1, 11)]]

def alpha_keyed(items):
    total = items.sum(axis=1)
    r = items.apply(lambda col: col.corr(total - col))   # item-rest correlation
    keys = np.where(r < 0, -1, 1)                        # flip reverse items
    scored = items * keys
    k = scored.shape[1]
    a = (k / (k - 1)) * (1 - scored.var().sum() / scored.sum(axis=1).var())
    return dict(alpha=a, keys=keys, scored=scored)

a = alpha_keyed(X)
a["keys"]                         # -1 marks a reverse-scored item
round(float(a["alpha"]), 2)       # alpha for the keyed scale
```
:::

A healthy $\alpha$. Now the classic "should I cut this item?" diagnostic - recompute $\alpha$ with each item *deleted*:

::: {.panel-tabset}
### R
```{r}
tibble(item = names(X)) |>
  mutate(alpha_if_deleted = map_dbl(item, \(j)
    alpha_keyed(select(X, -all_of(j)))$alpha)) |>
  round2()
```
### SPSS
```
* "Alpha if item deleted" - the classic should-I-cut-this-item table.
* RELIABILITY reports it directly with /SUMMARY=TOTAL; the column is
* headed "Cronbach's Alpha if Item Deleted".
RELIABILITY
  /VARIABLES=x1 x2 x3 x4 x5 x6 x7 x8 x9 x10
  /MODEL=ALPHA
  /SUMMARY=TOTAL.
```
### Julia
```{julia}
# Each tab runs in a fresh session, so the data and the helper come back
# with it - unchanged from the tab above.
using CSV, DataFrames, Statistics
d = CSV.read("data/moddat2.csv", DataFrame)
X = select(d, r"^x\d+$")

function alpha_keyed(items)
    total = sum(eachcol(items))
    r = [cor(col, total .- col) for col in eachcol(items)]
    keys = [x < 0 ? -1 : 1 for x in r]
    scored = DataFrame([n => c .* k for ((n, c), k) in zip(pairs(eachcol(items)), keys)])
    k_ = ncol(scored)
    a = (k_ / (k_ - 1)) *
        (1 - sum(var(c) for c in eachcol(scored)) / var(sum(eachcol(scored))))
    (alpha = a, keys = keys, scored = scored)
end

DataFrame(item = names(X),
          alpha_if_deleted = [round(alpha_keyed(select(X, Not(n))).alpha, digits = 2)
                              for n in names(X)])
```
### Python
```{python}
# Each tab runs in a fresh session, so the data and the helper come back
# with it - unchanged from the tab above.
import pandas as pd, numpy as np

d = pd.read_csv("data/moddat2.csv")
X = d[[f"x{i}" for i in range(1, 11)]]

def alpha_keyed(items):
    total = items.sum(axis=1)
    r = items.apply(lambda col: col.corr(total - col))
    keys = np.where(r < 0, -1, 1)
    scored = items * keys
    k = scored.shape[1]
    a = (k / (k - 1)) * (1 - scored.var().sum() / scored.sum(axis=1).var())
    return dict(alpha=a, keys=keys, scored=scored)

pd.DataFrame({"item": X.columns,
              "alpha_if_deleted": [round(alpha_keyed(X.drop(columns=j))["alpha"], 2)
                                   for j in X.columns]})
```
:::

Every "alpha-if-deleted" value sits *below* the full-scale $\alpha$, so no item is dead weight - each one, even the weakest, adds more signal than noise. (Had any value come out *above* the full $\alpha$, that item would be a candidate for the chopping block: removing it would make the scale *more* reliable.)

## Why Reliability Buys You Power

Now the payoff, and the link back to [statistical power](09-statisticalpower.qmd). Detection is the between-group signal over the within-group noise, and unreliability *is* that noise ($S^2_{error}$). Build the keyed scale score, then ask a real question of it - do women and men differ? - sizing the effect with Cohen's $d$:

::: {.panel-tabset}
### R
```{r}
score <- rowSums(a$scored)                 # the keyed total score

EScalc <- function(dv, cond) {             # McKnight's teaching function for Cohen's d
  parts <- tibble(dv = dv, cond = as_factor(cond)) |>
    summarise(m = mean(dv), v = var(dv), .by = cond) |>
    arrange(cond)
  (parts$m[1] - parts$m[2]) / sqrt((parts$v[1] + parts$v[2]) / 2)
}

tibble(cohens_d = EScalc(score, d$sex)) |> round2()
```
### SPSS
```
* Cohen's d for the keyed scale score between two groups.
* SPSS 27+ reports standardised effect sizes directly from T-TEST.
COMPUTE score = SUM(x1 TO x10).      /* using the keyed items */
EXECUTE.

T-TEST GROUPS=sex('M' 'F')
  /VARIABLES=score
  /ES DISPLAY(TRUE).
```
### Julia
```{julia}
# Fresh session again: rebuild the keyed scale, then size the difference.
using CSV, DataFrames, Statistics
d = CSV.read("data/moddat2.csv", DataFrame)
X = select(d, r"^x\d+$")

function alpha_keyed(items)
    total = sum(eachcol(items))
    r = [cor(col, total .- col) for col in eachcol(items)]
    keys = [x < 0 ? -1 : 1 for x in r]
    scored = DataFrame([n => c .* k for ((n, c), k) in zip(pairs(eachcol(items)), keys)])
    k_ = ncol(scored)
    a = (k_ / (k_ - 1)) *
        (1 - sum(var(c) for c in eachcol(scored)) / var(sum(eachcol(scored))))
    (alpha = a, keys = keys, scored = scored)
end

a = alpha_keyed(X)
score = sum(eachcol(a.scored))            # the keyed total score

function EScalc(dv, cond)                 # Cohen's d, pooled SD
    lv = unique(cond)                     # groups in order of appearance, as R does
    x1 = dv[cond .== lv[1]]; x2 = dv[cond .== lv[2]]
    (mean(x1) - mean(x2)) / sqrt((var(x1) + var(x2)) / 2)
end

round(EScalc(score, d.sex), digits = 2)
```
### Python
```{python}
# Fresh session again: rebuild the keyed scale, then size the difference.
import pandas as pd, numpy as np

d = pd.read_csv("data/moddat2.csv")
X = d[[f"x{i}" for i in range(1, 11)]]

def alpha_keyed(items):
    total = items.sum(axis=1)
    r = items.apply(lambda col: col.corr(total - col))
    keys = np.where(r < 0, -1, 1)
    scored = items * keys
    k = scored.shape[1]
    a = (k / (k - 1)) * (1 - scored.var().sum() / scored.sum(axis=1).var())
    return dict(alpha=a, keys=keys, scored=scored)

a = alpha_keyed(X)
score = a["scored"].sum(axis=1)           # the keyed total score

def EScalc(dv, cond):                     # Cohen's d, pooled SD
    lv = pd.unique(cond)                  # order of appearance, as R does - NOT sorted
    x1, x2 = dv[cond == lv[0]], dv[cond == lv[1]]
    return (x1.mean() - x2.mean()) / np.sqrt((x1.var() + x2.var()) / 2)

round(float(EScalc(score, d.sex)), 2)
```
:::

A large $d$. But the point is *why* it was detectable: because the scale was reliable. Had $\alpha$ been 0.5 instead, half the total-score variance would be pure error, the groups would blur together, and the same real difference could sink below significance. **A reliable measure is the cheapest power you will ever buy**: you pay once, in the design, and it pays back in every analysis that follows. (In practice you'd reach for `psych::alpha(X, check.keys = TRUE)`, which does the keying, the drop statistics, and much more in one call - but now you know exactly what it is doing under the hood.)

::: {.callout-important}
## The Dartboard: Consistency ≠ Accuracy

Picture throwing darts. **Reliability** is how *tightly grouped* your throws are - consistency. A tight cluster is reliable *even if it is nowhere near the bullseye*. That "near the bullseye" part - accuracy, being on target - is **validity**, and it is a completely separate question. A bathroom scale that always reads five pounds heavy is perfectly reliable and perfectly wrong. Reliability is *necessary* for validity (you cannot hit the bullseye if your throws scatter randomly) but nowhere near *sufficient*. That is the whole reason the next chapter exists.
:::

## Sources of Variance: A Glimpse of Generalizability Theory

Classical test theory splits observed variance into just two pieces, true and error: $S^2_{obs} = S^2_{true} + S^2_{error}$. But "error" is a bucket, and often we want to know *what is in it*. Is the noise coming from the particular *items* we chose? The *rater* who scored the responses? The *day* the person happened to show up? Each of these is a **source of variance**, and lumping them all into one "error" term hides exactly the information you would need to improve the measure.

**Generalizability theory** (G-theory) opens the bucket. It treats each of these as a **facet** and estimates how much of the total variance each one contributes, so you can see which sources matter most and where to spend your effort. As the authors put it, the whole basis of G-theory is to estimate the sources of variability and determine which ones affect your results most. Reliability stops being one number and becomes an analysis of *where the noise comes from* - and, in the follow-up "decision study," of how many items or raters you would need to pin the true score down. We build that machinery (variance components, the generalizability coefficient, the D-study) in the [item response and generalizability chapter](30-irt.qmd). For now, carry the idea: the $O = T + E$ split is the first step; naming the facets hidden inside that error is the next.

::: {.callout-note}
## Beyond Classical Test Theory

Everything in this chapter lives inside **classical test theory** - the $O = T + E$ world, where "reliability" is a single number for a whole test. It is the right place to start, but it has a blind spot: it treats every item as interchangeable and pins a person's score to the particular test they happened to take. Modern measurement - **item response theory** and its **Rasch** model - drops that assumption, estimating each *item's* difficulty and each *person's* ability on a common scale, so that reliability can vary across the range of the trait. We flag it here as the road ahead; the intuitions you build with $O = T + E$ are exactly the ones that make item response theory click later.
:::

## Challenge

::: {.callout-tip}
## Do One Yourself

1.  Simulate true scores and add three different amounts of error (small, medium, large). Compute reliability each time. Plot reliability against error variance - what is the shape?
2.  Build an 8-item scale where four items share a common factor and four are pure noise. Compute Cronbach's alpha for all 8 items, then for just the good 4. What happens, and what does it warn you about trusting alpha alone?
3.  Re-run the attenuation demo with *more* reliable sensors (smaller measurement error). Confirm the observed correlation creeps back toward the true value.
:::

## Where We Go Next

A reliable sensor is consistent, but consistency is worthless if it is consistently measuring the *wrong thing*. That is the bullseye question - are we measuring what we intend? - and it is the subject of [validity](11-validity.qmd).
