...
ggplot(h, aes(OPS)) +
  ...
  # a normal with this data's own mean and SD - here the curve is a REFERENCE,
  # not a claim: the gap between bars and curve is the interesting part
  stat_function(fun = dnorm, args = list(mean = mean(h$OPS), sd = sd(h$OPS)),
                colour = "grey30", linewidth = 0.9, linetype = "dashed") +
  geom_vline(xintercept = median(h$OPS), color = "firebrick", linewidth = 1) +
  labs(x = "OPS", y = "density", title = "Most hitters are ordinary; a few are stars") +
  theme_book()
```
### SPSS
```
