...
fork = CSV.read("data/sim/ch24-fork.csv", DataFrame)
...
fork = pd.read_csv("data/sim/ch24-fork.csv")
...
```{julia}
using CSV, DataFrames, Statistics, GLM
pipe = CSV.read("data/sim/ch24-pipe.csv", DataFrame)
...
import numpy as np, pandas as pd
import statsmodels.formula.api as smf
pipe = pd.read_csv("data/sim/ch24-pipe.csv")

pipe.X.corr(pipe.Y)                       # the real, total effect
np.corrcoef(smf.ols("X ~ M", pipe).fit().resid,
            smf.ols("Y ~ M", pipe).fit().resid)[0, 1]   # control M: effect blocked
```
:::
...
```{julia}
using CSV, DataFrames, Statistics, GLM
collider = CSV.read("data/sim/ch24-collider.csv", DataFrame)
...
import numpy as np, pandas as pd
import statsmodels.formula.api as smf
collider = pd.read_csv("data/sim/ch24-collider.csv")
